...
producePDFA = True #If true the final file will be prepress quality PDF/A-3b file otherwise ebook quality PDF file
...
def getocrResult(results):
    ...
    if len(results)>0: #0 lenght is returned if a file is dropped do to being empty
        ...
        if composeSingleFile:
            filename = results[1]    
            #printmessage("So far so good {}".format(filename))
            finalFiles.append(filename)
        #print(finalFiles) #This face everything still ok
...
#printmessage(str(pathList))

finalFiles = [] #Defines it globally outside the loop
...
for singlePath in pathList:
    ...
    for item in dirContent:        
        ...
        if end in fileformatresult:        
            ...
            if end != 'pdf': #If called with image file               
                #printmessage(item)
                pool.apply_async(ocrmyfile, args=(templist[0],), callback=getocrResult)                
    ...
    pool.join()
    if composeSingleFile:
        #finalFilePath = os.path.abspath(templist[0])
        #printmessage("Path to place final file = {}".format(finalFilePath))
        tempDisposeList = []
